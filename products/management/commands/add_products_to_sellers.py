    ...
    def handle(self, *args, **options):
        ...
        for seller_data in sellers_data:
            seller, created = User.objects.get_or_create(
                ...
                defaults={"email": seller_data["email"], "is_seller": True},
            )
            if created:
                seller.set_password("sellerpass123")
                seller.save()
                self.stdout.write(
                    f"✓ Created seller: {seller_data['name']} ({seller_data['username']})"
                )
            else:
                self.stdout.write(
                    f"✓ Seller exists: {seller_data['name']} ({seller_data['username']})"
                )
            ...
            for product_data in products:
                if options["force"]:
                    Product.objects.filter(
                        name=product_data["name"], seller=seller
                    ).delete()
        ...
        self.stdout.write(
            f"👨‍💼 Sellers: {User.objects.filter(is_seller=True).count()}"
        )
        ...
        self.stdout.write(
            "🌐 You can now browse the marketplace at http://127.0.0.1:8000/products/"
        )
