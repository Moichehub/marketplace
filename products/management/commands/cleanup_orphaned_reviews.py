...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if dry_run:
            self.stdout.write(
                self.style.WARNING(f"DRY RUN: Would delete {count} orphaned reviews:")
            )
            for review in orphaned_reviews[:10]:
                self.stdout.write(
                    f'  - Review ID {review.id} for product "{review.product.name}"'
                )
            if count > 10:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"Successfully deleted {count} orphaned reviews")
            )
