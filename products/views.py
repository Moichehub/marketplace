...
def product_detail(request, slug):
    ...
    try:
        reviews = (
            product.reviews.filter(user__isnull=False)
            .select_related("user")
            .order_by("-created_at")
        )
    except Exception as e:
    ...
    if (
        ...
        if form.is_valid():
            try:
                ...
                messages.error(
                    request, "Помилка при збереженні відгуку. Спробуйте ще раз."
                )
...
def edit_review(request, review_id):
    ...
    if request.method == "POST":
        form = ReviewForm(
            request.POST, instance=review, user=request.user, product=review.product
        )
        if form.is_valid():
            form.save()
...
def product_create(request):
    ...
    if request.method == "POST":
        ...
        form = ProductForm()
    return render(
        request, "products/product_form.html", {"form": form, "mode": "create"}
    )
...
def product_update(request, slug):
    ...
    try:
        ...
        messages.error(
            request,
            f"Товар з URL '{slug}' не знайдено або у вас немає прав для його редагування.",
        )
        return redirect("products:seller_dashboard")
...
def product_delete(request, slug):
    ...
    try:
        ...
        messages.error(
            request,
            f"Товар з URL '{slug}' не знайдено або у вас немає прав для його видалення.",
        )
        return redirect("products:seller_dashboard")
...
def create_sample_data(request):
    ...
    for username, email in [
        ...
        seller, created = User.objects.get_or_create(
            username=username, defaults={"email": email, "is_seller": True}
        )
        if created:
    ...
    products_data = [
        ...
        (
            "fashion",
            "Denim Jacket",
            "Classic denim jacket for any occasion",
            89.99,
            "Clothing",
        ),
        (
        ...
        (
            "fashion",
            "Sunglasses",
            "Trendy sunglasses with UV protection",
            159.99,
            "Clothing",
        ),
        (
        ...
        (
            "books",
            "Harry Potter Book 1",
            "J.K. Rowling magical first book",
            15.99,
            "Books",
        ),
        ...
        (
            "home",
            "LED Desk Lamp",
            "Adjustable LED lamp with multiple levels",
            49.99,
            "Home & Garden",
        ),
        (
        ...
        (
            "home",
            "Kitchen Mixer",
            "Professional stand mixer for baking",
            199.99,
            "Home & Garden",
        ),
        (
    ...
    messages.info(
        request,
        "Seller accounts created: techstore, fashion, books, home, sports (password: pass123)",
    )
    return redirect("products:list")
