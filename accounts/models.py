...
class SellerProfile(models.Model):
    ...
    store_name = models.CharField(
        max_length=200, unique=True, help_text="Назва вашого магазину"
    )
    store_slug = models.SlugField(
        unique=True, blank=True, help_text="URL-адреса магазину"
    )
    description = models.TextField(
        max_length=2000, blank=True, help_text="Опис магазину"
    )
    logo = models.ImageField(
        upload_to="store_logos/", blank=True, null=True, help_text="Логотип магазину"
    )
    ...
    payment_info = models.TextField(
        max_length=1000, blank=True, help_text="Інформація про способи оплати"
    )
    shipping_info = models.TextField(
        max_length=1000, blank=True, help_text="Інформація про доставку"
    )
    ...
    telegram = models.CharField(
        max_length=100, blank=True, help_text="Telegram username"
    )
    ...
    def total_sales(self):
        ...
        return (
            Order.objects.filter(items__product__seller=self.user, status="paid")
            .distinct()
            .count()
        )
